...
const ModalChef = ({mutate}) => {
    ...
    const handleSubmit=async()=>{
        ...
        if(email&&name&&bankAccount){
            const res=await axios.post("http://localhost:8082/api/v1/chefs",data)
            console.log(res);
            if(res.status==201){
                setEmail("")
                setName("")
                ...
                {(onClose) => (
                  <>
                    <form onSubmit={handleSubmit}>
                        ...
                        <ModalHeader className="flex flex-col gap-1">Add Chef</ModalHeader>
                        <ModalBody>
                            <Input
                                required
                                autoFocus
