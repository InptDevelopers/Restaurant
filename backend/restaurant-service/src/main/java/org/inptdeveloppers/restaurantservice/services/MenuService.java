...
public class MenuService {
    ...
    public MenuDTO getmenu(Long id, int page, int size) {
        Page<Item> items;
        if (menuRepository.getByRestaurant(restaurantRepository.findById(id).orElse(null)) == null) {
            return null;

        }
        Menu menu = menuRepository.getByRestaurant(restaurantRepository.findById(id).get());
        items = itemRepository.findByMenu(menu, PageRequest.of(page, size));
    ...
    public void createMenu(MenucreationDTO menucreationDTO) {
        ...
        Menu menu;
        Restaurant restaurant = restaurantRepository.findById(menucreationDTO.getIdrestaurant()).orElse(null);
